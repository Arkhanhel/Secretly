...
void main() {
  ...
  group('формы и тени по исходнику', () {
    ...
    test('🔴 СВЕЧЕНИЯ ПОД СВОИМ ПУЗЫРЁМ НЕТ — указание владельца 23.09.2026', () {
      // Оно стояло по макету и задумывалось как способ отделить своё от чужого,
      // не рисуя линий. На деле своё и так отличается заливкой и стороной, а
      // мягкое пятно под КАЖДЫМ своим сообщением складывается вдоль правого
      // края в сплошную кайму.
      //
      // Проверка держит именно ОТСУТСТВИЕ: макет сюда однажды уже приводил, и
      // без неё свечение вернулось бы «по макету» при следующей сверке.
      final bubble = File(
        'lib/ui/desktop/chat/message_bubble.dart',
      ).readAsStringSync();
      expect(bubble.contains('glowSelfBubble'), isFalse);
    });

    test('свечение активной плитки рейки на месте', () {
      // Второе свечение из макета трогать не просили: плитка одна, в кайму оно
      // не складывается и отвечает на вопрос «где я сейчас».
      final rail = File(
        'lib/ui/desktop/shell/sidebar.dart',
      ).readAsStringSync();
