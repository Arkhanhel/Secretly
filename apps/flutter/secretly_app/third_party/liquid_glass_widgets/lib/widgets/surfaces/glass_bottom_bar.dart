...
class JellyClipper extends CustomClipper<Path> {
  ...
  Path getClip(Size size) {
    ...
    // SECRETLY FORK (2026-07-23): drop the 4px HORIZONTAL inset of the label
    // colour mask. The mask was 4px narrower than the tab slot on each side, so a
    // long selected label that fills the slot had its leftmost letter fall just
    // outside the mask — the antiAliasWithSaveLayer seam painted that edge with
    // the unselected (white) colour ("невидимый ободок" on the letter). Keeping
    // the mask full-slot-width lets the accent colour cover the whole label; the
    // visible pill is drawn separately (AnimatedGlassIndicator) and is unchanged.
    // Vertical inset kept so the mask still matches the pill's height.
    final paddedRect = Rect.fromLTRB(
      baseRect.left - 6.0, // was +4; extend 6px PAST the slot so even the longest
      baseRect.top + 4.0, // Top padding
      baseRect.right + 6.0, // label («Настройки») is fully inside the colour mask
